...
class LatentConditioningStack(nn.Module):
    """
    Latent conditioning stack from DGMR to generate latent noise.
    Simplified version for testing purposes.
    """
    def __init__(self, input_channels=1, output_channels=384, num_layers=3):
        super().__init__()
    ...
    def forward(self, x):
        """
        Forward pass for latent conditioning.
        
        Args:
            x: Input tensor [B, C, T, H, W] or [B, T, C, H, W]
        
        Returns:
            Latent features [B, output_channels, H, W]
        """
        if x.dim() == 5:
            ...
            noise = torch.randn_like(x_latest) * 0.1
            x_latest = x_latest + noise
            
            # Encode latent
            latent = self.encoder(x_latest)
